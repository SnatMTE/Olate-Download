...
// Default data directory outside project root (parent of repo root)
$defaultOutside = dirname(__DIR__, 4) . DIRECTORY_SEPARATOR . 'olate_data';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    // Default data dir: place outside project root for security
    $defaultOutside = dirname(__DIR__, 4) . DIRECTORY_SEPARATOR . 'olate_data';

    // Allow installer to accept a custom data directory (for advanced users)
    $dataDir = isset($_POST['data_dir']) && trim($_POST['data_dir']) !== '' ? trim($_POST['data_dir']) : $defaultOutside;

    if (!is_dir($dataDir)) {
        if (!@mkdir($dataDir, 0755, true)) {
            $error = "Unable to create data directory ($dataDir). Check permissions or choose a different path.";
        }
    }

    // Ensure $dataDir is an absolute path
    $dataDir = realpath($dataDir) ?: $dataDir;
    $dbFile = rtrim($dataDir, "\/\\") . DIRECTORY_SEPARATOR . 'olate.sqlite';
    ...
    try {
        ...
        // Ensure we store the sqlite path so dbim can pick it up
        $dbPathForConfig = str_replace("\\", "\\\\", $dbFile); // escape backslashes for Windows paths

        $configContents = "<?php\n";
        $configContents .= "// Generated by SQLite installer\n";
        $configContents .= "if (!defined('DB_PREFIX')) define('DB_PREFIX', 'downloads_');\n\n";
        $configContents .= "\$config = array();\n";
        $configContents .= "\$config['database'] = array(\n";
        $configContents .= "\t'username' => '',\n";
        $configContents .= "\t'password' => '',\n";
        $configContents .= "\t'server'   => '',\n";
        $configContents .= "\t'name'     => '',\n";
        $configContents .= "\t'persistant'=> false,\n";
        $configContents .= "\t'sqlite_path' => '" . $dbPathForConfig . "',\n";
        $configContents .= ");\n";
        $configContents .= "?>\n";
...
<?php else: ?>
    <form method="post">
        ...
        <p><label>Data directory (recommended outside webroot): <input name="data_dir" size="60" value="<?php echo htmlspecialchars(isset($dataDir) ? $dataDir : $defaultOutside, ENT_QUOTES); ?>"></label></p>
