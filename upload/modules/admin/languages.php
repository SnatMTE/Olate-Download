	...
	if (is_array($_REQUEST['activate']) && sizeof($_REQUEST['activate']) > 0)
	{
		foreach ($_REQUEST['activate'] as $file => $value)
		{
			...
			if (file_exists($file_path) && !is_dir($file_path) && strpos($file, '.php') !== false)
			{
				...
				$search_row = $dbim->fetch_array_p($search);
				...
				if ($search_row['count'] == 0)
				{
					...
					$dbim->pquery('INSERT INTO '.DB_PREFIX.'languages
									SET name = ?,
										...
										version_major = ?,
										version_minor = ?',
									array($language_info['name'], $language_info['filename'],
										$language_info['version'][0], $language_info['version'][1]));
				}
				else 
				{
